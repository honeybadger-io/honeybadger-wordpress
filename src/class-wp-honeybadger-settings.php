...
class WP_Honeybadger_Settings {
    ...
    public function sanitize_js_api_key($input) {
        if (empty($input) || !is_string($input)) {
            add_settings_error('wp_honeybadger_settings', 'invalid-js-api-key', 'Invalid JS API Key');
            return '';
        }
    ...
    public function sanitize_php_api_key($input) {
        if (empty($input) || !is_string($input)) {
            add_settings_error('wp_honeybadger_settings', 'invalid-php-api-key', 'Invalid PHP API Key');
            return '';
        }
        return sanitize_text_field($input);
    }
    ...
    public function js_enabled_render() {
        ...
        ?>
        <input type='checkbox' name='wp_honeybadger_settings[wp_honeybadger_js_enabled]' <?php checked($options['wp_honeybadger_js_enabled'] ?? 1, 1); ?> value='1'>
        <?php
    }
    ...
    public function php_enabled_render() {
        $options = get_option('wp_honeybadger_settings');
        ?>
        <input type='checkbox' name='wp_honeybadger_settings[wp_honeybadger_php_enabled]' <?php checked($options['wp_honeybadger_php_enabled'] ?? 1, 1); ?> value='1'>
        <?php
    }
